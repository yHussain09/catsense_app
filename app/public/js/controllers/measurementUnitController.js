catSenseApp.controller('measurementUnitController', function ($scope, $http, $q) {
    self = $scope;
    let url = 'http://localhost:3000/api/v1/measurementUnits';
...
//   $scope.loadOrgaDataCombo();
  ...
  $scope.update = function (model) {
    $http.put(url + '/' + model.measurementUnitId, JSON.stringify(model))
      .then(function (success) {
        console.log(success);
        $scope.loadGrid();
      }, function (error) {
  ...
  $scope.delete = function (model) {
    $http.delete(url + '/' + model.measurementUnitId)
      .then(function (success) {
        console.log(success);
        $scope.loadGrid();
      }, function (error) {
  ...
  $scope.setPageSize = function (pageSize) {
    ...
    let gridDataSource = new DevExpress.data.CustomStore({
        ...
        update: function (key, values) {
            return $http.put(url + '/' + encodeURIComponent(key.measurementUnitId), JSON.stringify(values))
                .then(function (response) {
                    console.log(response);
                    // $scope.reloadGrid();
                    return {
        ...
        remove: function (key) {
            return $http.delete(url + '/' + encodeURIComponent(key.measurementUnitId))
                .then(function (response) {
                    console.log(response);
