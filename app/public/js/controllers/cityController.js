catSenseApp.controller('cityController', function($scope, $http, $q) {
    self=$scope;
  ...
  $scope.update = function (model) {
    $http.put(url + '/' + model.countryId, JSON.stringify(model))
      .then(function (success) {
        console.log(success);
        $scope.loadGrid();
      }, function (error) {
  ...
  $scope.delete = function (model) {
    $http.delete(url + '/' + model.countryId)
      .then(function (success) {
        console.log(success);
        $scope.loadGrid();
      }, function (error) {
  ...
  $scope.setPageSize = function (pageSize) {
    ...
    let gridDataSource = new DevExpress.data.CustomStore({
      ...
      update: function(key, values){
        return $http.put(url + '/' +  encodeURIComponent(key.cityId), JSON.stringify(values))
        .then(function (response) {
          console.log(response);
          // $scope.reloadGrid();
          return { data: response.data.rows, totalCount: response.data.count };
      ...
      remove: function(key){
        return $http.delete(url + '/' + encodeURIComponent(key.cityId))
        .then(function (response) {
          console.log(response);
    ...
    $scope.dataGridOptions = {
      ...
      columns: [ 
        ...
        {caption: 'City', dataField:'name'}, {caption: 'Description', dataField:'description'}
      ],
      showBorders:true,
