catSenseApp.controller('productController', function($scope, $http, $q) {
    self=$scope;
    let url = 'http://localhost:3000/api/v1/products';
    let orgaDataComboUrl = 'http://localhost:3000/api/v1/organizations/dataCombo';
    let prodTypeDataComboUrl = 'http://localhost:3000/api/v1/productTypes/dataCombo';
    let measureUniDataComboUrl = 'http://localhost:3000/api/v1/measurementUnits/dataCombo';
  ...
  $scope.prodTypeDataCombo = [];
  $scope.measureUniDataCombo = [];
  $scope.fixedCombo = [{
      code: "Y",
      description: "Active"
    },
  ...
  $scope.loadProdTypeDataCombo = function () {
    $http({
      method: 'GET',
      url: prodTypeDataComboUrl
    }).then(function mySuccess(response) {

      angular.forEach(response.data, function (item, index) {
        let itemObject = {};
        itemObject.code = item.productTypeId;
        itemObject.description = item.name;
        itemObject.organizationId = item.organizationId;
        $scope.prodTypeDataCombo.push(itemObject);
      });
      console.log(response);
    }, function myError(response) {
      console.log(response);
  ...
  $scope.loadMeasureUniDataCombo = function () {
    $http({
      method: 'GET',
      url: measureUniDataComboUrl
    }).then(function mySuccess(response) {

      angular.forEach(response.data, function (item, index) {
        let itemObject = {};
        itemObject.code = item.measurementUnitId;
        itemObject.description = item.name;
        itemObject.organizationId = item.organizationId;
        $scope.measureUniDataCombo.push(itemObject);
      });
      console.log(response);
    }, function myError(response) {
      console.log(response);
  ...
  $scope.loadProdTypeDataCombo();
  $scope.loadMeasureUniDataCombo();
  $scope.loadGrid();
  ...
  $scope.update = function (model) {
    $http.put(url + '/' + model.productId, JSON.stringify(model))
      .then(function (success) {
        console.log(success);
        $scope.loadGrid();
      }, function (error) {
  ...
  $scope.delete = function (model) {
    $http.delete(url + '/' + model.productId)
      .then(function (success) {
        console.log(success);
        $scope.loadGrid();
      }, function (error) {
  ...
  $scope.setPageSize = function (pageSize) {
    ...
    let gridDataSource = new DevExpress.data.CustomStore({
      ...
      update: function(key, values){
        return $http.put(url + '/' + encodeURIComponent(key.productTypeId), JSON.stringify(values))
        .then(function (response) {
          console.log(response);
          // $scope.reloadGrid();
          return { data: response.data.rows, totalCount: response.data.count };
      ...
      remove: function(key){
        return $http.delete(url + '/' + encodeURIComponent(key.productTypeId))
        .then(function (response) {
          console.log(response);
    ...
    $scope.dataGridOptions = {
      ...
      columns: [
        ...
        },
        {
            ...
            lookup: {
              dataSource: prodTypeDataComboUrl,
              key: 'productTypeId',
        ...
        {caption: 'Quentity', dataField:'quentity'},
        {
            caption: 'Measurement Unit', 
            dataField:'measurementUnitId',
            lookup: {
              dataSource: measureUniDataComboUrl,
              key: 'measurementUnitId',
              valueExpr: 'measurementUnitId',
              displayExpr: 'name'
            }
        }
    ],
